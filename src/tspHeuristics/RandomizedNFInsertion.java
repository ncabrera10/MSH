...
public class RandomizedNFInsertion {
	...
	private static double[] dMaxs;
	
	/**
	 * This method creates an instance for the heuristic
	 */
	public RandomizedNFInsertion(OriginalGraph G,DataHandler data,int fact) {
		kk = fact;
		cost = 0.0;
		dMins = new double[G.getNumNodes()];
		dMaxs = new double[G.getNumNodes()];
	...
	public void updateDMinMax(OriginalGraph graph) {
		for(Iterator<Integer> iter = nodesList1.iterator();iter.hasNext();) {
			...
			for(Iterator<Integer> iter2 = nodesList2.iterator();iter2.hasNext();) {
				...
				if(actNode!=actNode2) {
					...
					if(graph.getDists()[actNode2][actNode] > dMaxs[actNode]) {
						dMaxs[actNode] = graph.getDists()[actNode2][actNode];
					}
				}
			}
		}
		...
		//System.out.println(ran+" - "+nodesList1.toString());
	}
	...
	public static void sortDMax(ArrayList<Integer>set) {
		QS2(set,0,set.size()-1);
	}
	
	public static void QS2(ArrayList<Integer>e, int b,int t) {
		int pivote;
	     if(b < t){
	...
	public static int colocar2(ArrayList<Integer>e,int b,int t) {
		int i;
	    int pivote;
	    double valor_pivote;
	    Integer temp;
	    //System.out.println("Encontre  al colocar");
	    pivote = b;
	    valor_pivote = dMaxs[e.get(pivote)];
	    //System.out.println((e.get(pivote))+" - "+valor_pivote);
	    for (i=b+1; i<=t; i++){
	        if (dMaxs[e.get(i)] > valor_pivote){
	                pivote++;    
	                temp= e.get(i);
	                e.set(i, e.get(pivote));
	                e.set(pivote, temp);
