...
public class RandomizedBestInsertionI {
	 ...
	 */
	public RandomizedBestInsertionI(OriginalGraph G,DataHandler data,int fact) {
		kk = fact;
		cost = 0.0;
	...
	public void updateDMin(OriginalGraph graph) {
		for(Iterator<Integer> iter = nodesList1.iterator();iter.hasNext();) {
			...
			dMins[actNode] = Double.POSITIVE_INFINITY;
			for(Iterator<Integer> iter2 = nodesList2.iterator();iter2.hasNext();) {
				int actNode2 = iter2.next();
				if(actNode!=actNode2) {
					for(Iterator<Integer> iter3 = nodesList2.iterator();iter3.hasNext();) {
						int actNode3 = iter3.next();
						if(actNode3!=actNode2 && actNode3 != actNode) {
							
							if(actNode2 != -1 && actNode3!= -1) {
								if(graph.getDists()[actNode2][actNode] + graph.getDists()[actNode][actNode3] - graph.getDists()[actNode2][actNode3]  < dMins[actNode]) {
									dMins[actNode] = graph.getDists()[actNode2][actNode] + graph.getDists()[actNode][actNode3] - graph.getDists()[actNode2][actNode3];
								}
							}
							else if(actNode2 == -1) {
								if(graph.getDists()[actNode][graph.getNumNodes()-1] + graph.getDists()[actNode][actNode3] - graph.getDists()[actNode3][graph.getNumNodes()-1]  < dMins[actNode]) {
									dMins[actNode] = graph.getDists()[actNode][graph.getNumNodes()-1] + graph.getDists()[actNode][actNode3] - graph.getDists()[actNode3][graph.getNumNodes()-1];
								}
							}
							else if(actNode3 == -1) {
								if(graph.getDists()[actNode2][actNode] + graph.getDists()[actNode][graph.getNumNodes()-1] - graph.getDists()[actNode2][graph.getNumNodes()-1]  < dMins[actNode]) {
									dMins[actNode] = graph.getDists()[actNode2][actNode] + graph.getDists()[actNode2][graph.getNumNodes()-1] - graph.getDists()[actNode2][graph.getNumNodes()-1];
								}
							}
